...
int octavei=0;
float notes[12]={};           // for piezo
float midiNotes[12]={};       // for midi

// initilize notes
int noteIndex=12;             // for piezo
float midiNotesList[12]={};   // for midi
...
void loop() {
  ...
  //Determine Note(s)
  noteIndex=12;              //reset
  for (int i=0;i<12;i++) { midiNotesList[i]=0;} //reset
  ...
  determineNotes(analogPin1Reading,analogPin2Reading,analogPin3Reading,analogPin4Reading);
  ...
  if (pinModeReading==HIGH && noteIndex!=12) {
    ...
    if (midiNotesList[0]!=0) {
      ...
      pause(analogPin1Reading,analogPin2Reading,analogPin3Reading,analogPin4Reading);
    }
  }
...
void determineNotes(int analogPin1Reading, int analogPin2Reading, int analogPin3Reading, int analogPin4Reading) {
  int j=0;
...
void pause(int analogPin1Reading, int analogPin2Reading, int analogPin3Reading, int analogPin4Reading) {
  // The current state is ended either with a change in 4 analog readings or the potentiometer reading
  ...
  int newPotReading=analogRead(potPin);delay(1);
