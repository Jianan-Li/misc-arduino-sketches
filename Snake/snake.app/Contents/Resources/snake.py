...
def timerFired(canvas):
    ...
    redrawAll(canvas)
    delay = 150-canvas.data['snakelength']*3 # ms
...
def drawSnakeCell(canvas, snakeBoard, row, col):
    ...
    if (snakeBoard[row][col] > 0):
        ...
        if [row,col]==canvas.data['foodposition']:
            ...
            while True:
                foodcolor=random.choice(['lavender','LawnGreen','LemonChiffon','coral','DarkBlue','ForestGreen','gold','GreenYellow','HotPink','BlueViolet','beige','azure','brown','red','green','white','purple','cyan','magenta','orange'])
